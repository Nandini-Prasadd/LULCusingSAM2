...
NOTEBOOK_TITLE = 'Parameters FLOPs Memory and Latency'
NEED_BASE = True
NEED_TINY = True
...
out_dir = pathlib.Path("/kaggle/working/results/05_computational_cost")

for label, run_dir in [("tiny", tiny_run), ("base_plus", base_run)]:
    target = out_dir / label
    common = [
        "benchmark", "--sam2-repo", SAM2_DIR,
        "--run-dir", run_dir,
        "--val-images", VAL_IMAGES, "--val-masks", VAL_MASKS,
        "--out-dir", target, "--eval-batch-size", "1", "--workers", "2",
        "--warmup", "5", "--runs", "30", "--profile-flops",
    ]
    run_runner(*common)
    run_runner(*common, "--use-tta")

ensemble_target = out_dir / "ensemble"
ensemble_common = [
    "benchmark-ensemble", "--sam2-repo", SAM2_DIR,
    "--base-run-dir", base_run, "--tiny-run-dir", tiny_run,
    "--val-images", VAL_IMAGES, "--val-masks", VAL_MASKS,
    "--out-dir", ensemble_target, "--eval-batch-size", "1", "--workers", "2",
    "--warmup", "5", "--runs", "30", "--profile-flops",
]
run_runner(*ensemble_common)
run_runner(*ensemble_common, "--use-tta")
archive_results(out_dir, "rsase_computational_cost")